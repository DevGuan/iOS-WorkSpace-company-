...
public class CYLSparkLoading: UIView {
    
    static var share :CYLSparkLoading? = nil
...
//        showLoading()
   ...
   static public func showLoading() {
        share = CYLSparkLoading.init(frame: UIScreen.main.bounds)
        UIApplication.shared.keyWindow?.addSubview(share!)
        share?.shouldLoadingDone = false
        share?.curStatus = .loading
    }
    
   public func dismissLoading(status:LoaderStatus) {
        curStatus = status
        shouldLoadingDone = true
    }
    
   static public func dismissSuccessful() {
        share?.curStatus = .done
        share?.shouldLoadingDone = true
    }
    
   static public func dismissFailed() {
        share?.curStatus = .failed
        share?.shouldLoadingDone = true
    }
    ...
    func drawLoadingDone() {
        ...
        if #available(iOS 9.0, *) {
            ...
            anim.toValue = 1
            anim.delegate = self
            anim.setValue(statusLayerScaleAnim, forKey: animNameKey)
            statusLayer.add(anim, forKey: nil)
        }
    }
